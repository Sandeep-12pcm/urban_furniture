...
export function SalesOrderDetail({ id, navigate }) {
  ...
  async function loadOrder() {
    ...
    setError(null);
    try {
      const data = await api.salesOrders.get(id);
      setOrder(data);
    } catch (err) {
  ...
  async function handleConfirm() {
    ...
    try {
      const updated = await api.salesOrders.confirm(id);
      setOrder(updated);
      success(`Sales Order "${updated.orderNumber}" confirmed!`);
    } catch (err) {
      toastError(err.message || 'Failed to confirm sales order.');
    } finally {
  ...
  async function handleCreateInvoice() {
    setActionLoading(true);
    try {
      const inv = await api.salesOrders.createInvoice(id);
      success(`Customer invoice "${inv.invoiceNumber}" generated from order!`);
  ...
  if (loading) {
    return (
      <div className="py-24">
        <LoadingSpinner size="lg" message="Loading sales order details..." />
      </div>
    );
  }
  ...
  if (error || !order) {
    return (
      <div>
        ...
        <ErrorState message={error || 'Order not found'} onRetry={loadOrder} />
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-12">
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            {order.status === 'CONFIRMED' && order.invoiceStatus !== 'FULLY INVOICED' && (
              <Button
                variant="success"
                icon={FilePlus2}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-1 font-bold text-slate-900 text-base">{order.customerName}</div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <span className="text-xs text-slate-400 font-semibold uppercase">Invoicing Status</span>
          <div className="mt-1">
            <Badge
              variant={
