...
export function SalesOrdersPage() {
  ...
  const salesOrdersApi = { list: salesApi.orders, listKey: 'salesOrders' };
  const params = { search: debouncedSearch || undefined, status: statusFilter || undefined, page };
  const { data: orders, pagination, loading, error, reload } = useResourceList(salesOrdersApi, params, 'salesOrders');

  async function confirmOrder(order) {
    setConfirmingId(order.id);
    try {
      await salesApi.confirmOrder(order.id);
      notify(`Sales Order ${order.orderNumber} confirmed.`);
      reload();
    } catch (requestError) {
  ...
  async function handleCancel() {
    setCancelling(true);
    try {
      await salesApi.cancelOrder(cancelTarget.id);
      notify(`Sales Order ${cancelTarget.orderNumber} cancelled.`);
  ...
  return (
    <div>
      <PageHeader
        title="Sales Orders"
        subtitle="Commercial customer orders — no accounting impact until invoiced"
        actions={
      ...
      <div className="rounded-2xl border border-white/80 bg-white p-5 shadow-card">
        ...
        {!loading && !error && orders.length === 0 && (
          <EmptyState icon={ShoppingCart} title="No sales orders found." action={<Button type="button" onClick={() => setFormOpen(true)}><Plus className="h-4 w-4" />New Sales Order</Button>} />
        )}
...
function SalesOrderFormModal({ open, onClose, onSaved }) {
  const { notify } = useToast();
  const [customers, setCustomers] = useState([]);
  const [products, setProducts] = useState([]);
  const [form, setForm] = useState({ customerId: '', orderDate: new Date().toISOString().slice(0, 10), expectedDate: '', reference: '', lines: [emptyLine()] });
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    if (open) {
      contactsApi.list({ type: 'CUSTOMER', status: 'ACTIVE', limit: 100 }).then((d) => setCustomers(d.contacts || []));
      contactsApi.list({ type: 'BOTH', status: 'ACTIVE', limit: 100 }).then((d) => setCustomers((current) => [...current, ...(d.contacts || [])]));
      productsApi.list({ status: 'ACTIVE', limit: 200 }).then((d) => setProducts(d.products || []));
      setForm({ customerId: '', orderDate: new Date().toISOString().slice(0, 10), expectedDate: '', reference: '', lines: [emptyLine()] });
      setError('');
    }
  }, [open]);
  ...
  async function submit(event) {
    ...
    try {
      ...
      setError(requestError.message || 'Could not create Sales Order.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <Modal open={open} title="New Sales Order" onClose={onClose} width="max-w-3xl">
