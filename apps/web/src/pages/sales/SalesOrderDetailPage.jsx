...
export function SalesOrderDetailPage() {
  ...
  async function load() {
    ...
    setError('');
    try {
      const data = await salesApi.order(id);
      setOrder(data.salesOrder);
    } catch (requestError) {
      setError(requestError.message || 'Unable to load this Sales Order.');
    } finally {
      setLoading(false);
    }
  }
  ...
  function startEdit() {
    ...
    Promise.all([
      contactsApi.list({ type: 'CUSTOMER', status: 'ACTIVE', limit: 100 }),
  ...
  async function saveEdit() {
    setSaving(true);
    try {
      const data = await salesApi.updateOrder(id, {
        customerId: editForm.customerId,
        orderDate: editForm.orderDate,
        expectedDate: editForm.expectedDate || undefined,
        reference: editForm.reference.trim() || undefined,
        items: editForm.lines.map((l) => ({ productId: l.productId, description: l.description || undefined, quantity: Number(l.quantity), unitPrice: Number(l.unitPrice), taxRate: Number(l.taxRate) })),
      });
      setOrder(data.salesOrder);
      notify('Sales Order updated successfully.');
      setEditing(false);
    } catch (requestError) {
      notify(requestError.message || 'Could not update Sales Order.', { tone: 'error' });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function confirmOrder() {
    setActing('confirm');
    try {
      const data = await salesApi.confirmOrder(id);
      setOrder(data.salesOrder);
  ...
  async function cancelOrder() {
    setActing('cancel');
    try {
      const data = await salesApi.cancelOrder(id);
      setOrder(data.salesOrder);
  ...
  async function createInvoice() {
    ...
    try {
      navigate(`/sales/invoices?fromOrder=${id}`);
    } finally {
  ...
  return (
    <div>
      <button type="button" onClick={() => navigate('/sales/orders')} className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-navy hover:text-indigo">
        <ArrowLeft className="h-4 w-4" />
        Back to Sales Orders
      </button>

      {loading && <div className="rounded-2xl border border-white/80 bg-white p-8 shadow-card"><InlineSpinner label="Loading Sales Order…" /></div>}
      ...
      <ConfirmDialog
        ...
        description={`"${order?.orderNumber}" will be cancelled and can no longer be confirmed or invoiced.`}
