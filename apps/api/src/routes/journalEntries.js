...
const { createDraftEntry, updateDraftEntry, postEntry, cancelDraft, getEntry, entrySelect, serializeEntry } = require('../services/accountingService');
...
function journalEntriesRoutes(db) {
 ...
 router.post('/journal-entries',...access,async(req,res,next)=>{try{const entry=await createDraftEntry(db,req.user.id,req.body);res.status(201).json({journalEntry:entry});}catch(e){sendServiceError(res,e);}});
